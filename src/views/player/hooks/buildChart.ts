...
export const buildBattlesEchart = (classifyShip: any, battlesEchart: echarts.ECharts) => {
  ...
  for (const type in classifyShip) {
    const dataList = []
    for (const key in classifyShip[type]) {
      dataList.push(classifyShip[type][key])
    }
    option.series.push({
      name: usePlayer().shipTypeList.find(e => {
        return e.shipType === type
      })?.typeName,
      type: 'bar',
      stack: 'battles',
      data: dataList
